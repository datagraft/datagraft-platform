...
docker exec -t datagraft-db bash -c 'until psql -h localhost --user=postgres --list| grep -q datagraft-test; do sleep 1; done'

docker exec datagraft-portal rake db:migrate

docker exec -i datagraft-db psql --user=postgres --dbname=datagraft-test -h localhost << EOF
